...
const useSendHandler = () => {
  ...
  const handleSend = async (text: string, setValue: (v: string) => void) => {
    setLoading(true);
...
const CustomSendAdvancedDemo: React.FC = () => {
  ...
  const sections = [
    {
      title: '示例1：完全自定义操作按钮',
      desc: '替换所有默认按钮，添加自定义的设置、云保存和发送按钮。',
      value: value1,
      setValue: setValue1,
      render: fullyCustomRender,
    },
    {
      title: '示例2：在默认按钮基础上添加自定义按钮',
      desc: '保留默认的发送按钮，同时添加自定义的点赞按钮。',
      value: value2,
      setValue: setValue2,
      render: mergeDefaultsRender,
    },
    {
      title: '示例3：简洁的自定义发送按钮',
      desc: '使用简单的圆角按钮替换默认发送图标。',
      value: value3,
      setValue: setValue3,
