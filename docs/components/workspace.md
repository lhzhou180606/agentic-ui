...
树形文件列表，目录节点在首次展开时通过 `onLoadChildren` 拉取子节点（懒加载）。`filterKeyword` 仅匹配已展开目录下的已加载子树；无匹配时按是否已展开展示不同文案（`workspace.treeFilterNoMatchVisibleRoots` / `workspace.treeFilterNoMatchInExpanded`）。与 `Workspace.File` 的 `fileTreeSwitch` 联用时由 `Workspace.File` 注入关键字。
